...
rabbits_killed_per_fox_birth = 1200 #The number of rabbits that must be killed for a fox to be born. (Rabbits/Fox)
...
rabbit_growth_rate = 0.50 # The percent of the rabbit population that will be born this time step. (1/Year)
fox_death_rate = 0.05 #The percent of the fox population that will die this time step from old age. (1/Year)
...
rabbit_plot = plot(x=model_time, y=rabbits_over_time/1000, Geom.line)
fox_plot    = plot(x=model_time, y=foxes_over_time, Geom.line)
draw(SVG("rabbit_plot.svg", 4inch, 3inch), rabbit_plot)
draw(SVG("fox_plot.svg", 4inch, 3inch), fox_plot)
